...
# Python 3.9
# Copyright (C) 2023 Justin Clancy justinpclancy@gmail.com
#
#
...
def gen_cold_clump_mask(nside = 2048, apodization = None, extent = 1.5):
    """
    ...
    apodization : 2D Gaussian profile apodization threshold - effectively
                  feathers the borders of the source masks, the higher
                  the threshold, the sharper the border. Setting this value to
                  None returns a circular binary mask
    extent      : For a binary mask, this value is how large the mask should 
                  be in relation to the major axis of the clump (default = 1.5)
    -----Ouputs----
    ...
    for i in range(NSources):
        if apodization is not None:
            ...
            for j in range(len(profile)):
                if profile[j] <= apodization:
                    profile[j] = 0
        elif apodization is None:
            pix_region = hp.query_disc(nside = nside,
                                       vec = vecs[i],
                                       radius = extent*fwhm_maj[i])
            # Binary mask profile
            profile = 0
